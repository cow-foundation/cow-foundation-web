...
const NavItem = ({
  ...
  return item.children ? (
    ...
    <RootNavItem mobileMode={mobileMode}>
      {href.startsWith('/#') && mobileMode && onAnchorLinkClick ? (
        // For anchor links in mobile mode, handle click manually
        <div
          onClick={(e) => {
            e.preventDefault()
            // Close the mobile menu immediately
            onAnchorLinkClick()
            // Navigate to the anchor after a small delay
            setTimeout(() => {
              window.location.hash = href.substring(2) // Remove the /# from href
            }, 100)
          }}
